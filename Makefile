...
linux_amd64:
	...
	@if [ -f config.yml ]; then cp config.yml build_staging_$@/; fi
	@cd build_staging_$@ && tar -czf ../$(BASE_DIR)/$(APP_NAME)_$@.tar.gz *
	@rm -rf build_staging_$@
...
windows_amd64:
	...
	@if [ -f config.yml ]; then cp config.yml build_staging_$@/; fi
	@cd build_staging_$@ && zip -q ../$(BASE_DIR)/$(APP_NAME)_$@.zip *
	@rm -rf build_staging_$@
